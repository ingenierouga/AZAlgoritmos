const obtenerGanador = (data) => {
  ...
  for (rondaCont = 1; rondaCont <= rondas; rondaCont++) {
    ...
    console.log("revisando la ronda: " + rondaCont);
    resRond = lineas[rondaCont].trim().split(" ");

    //se agrega el resultado de la ronda al total de cada jugador
    Jug1.puntuacion = Jug1.puntuacion + parseInt(resRond[0]);
    console.log("total jugador 1:" + Jug1.puntuacion);
    Jug2.puntuacion = Jug2.puntuacion + parseInt(resRond[1]);
    console.log("total jugador 2:" + Jug2.puntuacion);
    ...
    if (Jug1.puntuacion > Jug2.puntuacion) {
      ...
      console.log(
        "gano jugador 1 con ventaja de:" + (Jug1.puntuacion - Jug2.puntuacion)
      );
    } else {
      //una vez obtenido el jugador se revisa si la ventaja obtenida es la mayor
      verificarVentajaMayor(Jug2, Jug1);
      console.log(
        "gano jugador 2 con ventaja de:" + (Jug2.puntuacion - Jug1.puntuacion)
      );
    }
